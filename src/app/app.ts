...
@Component({
  selector: 'app-root',
  imports: [Navbar, Filter, Main, FormsModule],
...
export class App {
  ...
  extensions = [...this.main_extensions];

  getFilteredType(filterType: FilterActiveCards): void {
    if (filterType === 'all') {
      this.extensions = [...this.main_extensions];
    } else if (filterType === 'active') {
      this.extensions = this.main_extensions.filter(
        (extension: Iextentions) => extension.isActive === true
      );
    } else {
      this.extensions = this.main_extensions.filter(
        (extension: Iextentions) => extension.isActive === false
      );
    }
  }
}
